...
## 2.4 保存与付费（基础全屏水印模式）
- 未付费状态下（基础版）：无论何时生成并进入预览界面，系统均会自动对拼接完成的长图应用全屏覆盖、倾斜重复的文字水印（“图片来自 App Store滚动长截屏-滚动长截图”）。保存时同样会原封不动保存带水印的结果图。主界面包含“付费去水印”引导。
- 已购买状态下（Pro）：系统不再应用全屏水印，用户看到和保存的始终为无损原始长图。
- 没有免费试用期概念。
