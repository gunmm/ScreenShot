...
## 2.4 保存与付费（Soft Paywall）
- 免费试用期内或已购买（Pro）的用户：点击“保存”按钮，图片直接无水印保存至相册。
- 试用期结束且未购买的用户：点击“保存”时弹出软付费墙，提供以下选项：
  1. “带水印保存”：系统在图片右下角绘制水印（例如：“由滚动长截屏 App 生成”）后，将带水印的图片存入相册。
  2. “去解锁”：进入购买流程（支付 18 元）。购买成功后，即可直接无损保存。
