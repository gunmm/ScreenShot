...
- **模块与职责**：
  ...
  - MosaicMagnifierView：负责在绘制期间展示局部放大预览，并根据手指位置在左上/右上之间避让。
- **数据流**：
...
- 放大镜：单指开始拖动时立即显示 160 x 160 的方形放大镜；纯点击路径不应触发放大镜，从而避免双击缩放时闪现；采样区域按 3.0 倍放大；默认放在图片左上角，若手指进入其覆盖区域则切换到右上角，反之亦然，抬手后隐藏。
